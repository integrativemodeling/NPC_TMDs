...
class PerinuclearVolumeLocationRestraint(IMP.pmi.restraints.RestraintBase):
    """Keep protein bound to the half torus in the xy plane.
    """
    def __init__(self, hier, protein=None,
                 tor_R=540.0, tor_r=127.5, tor_th=45.0,
                 consider_radius = False, sigma=0.2,
                 resolution=1, label=None, weight=1.0):
        """Constructor
        """

        super(PerinuclearVolumeLocationRestraint, self).__init__(
            hier.get_model(), label=label, weight=weight)

        mex = IMP.npc.PerinuclearVolumeLocationRestraint(
            self.model, tor_R, tor_r, tor_th,
            consider_radius, sigma)
        residues = IMP.pmi.tools.select_by_tuple_2(
            hier, protein, resolution=resolution)
        for residue in residues:
            mex.add_particle(residue)
        ...
        self.dataset = None
        for p, state in IMP.pmi.tools._all_protocol_outputs(hier):
            p.add_perinuclear_volume_location_restraint(
                state, residues, tor_R, tor_r, tor_th, sigma, self)

        self.rs.add_restraint(mex)

class PoreSideVolumeLocationRestraint(IMP.pmi.restraints.RestraintBase):
    """Keep protein bound to the half torus in the xy plane.
    """
    def __init__(self, hier, protein=None,
                 tor_R=540.0, tor_r=127.5, tor_th=45.0,
        ...
        super(PoreSideVolumeLocationRestraint, self).__init__(
            hier.get_model(), label=label, weight=weight)

        mex = IMP.npc.PoreSideVolumeLocationRestraint(
            self.model, tor_R, tor_r, tor_th,
            consider_radius, sigma)
        residues = IMP.pmi.tools.select_by_tuple_2(
            hier, protein, resolution=resolution)
        ...
        self.dataset = None
        for p, state in IMP.pmi.tools._all_protocol_outputs(hier):
            p.add_poreside_volume_location_restraint(
                state, residues, tor_R, tor_r, tor_th, sigma, self)
